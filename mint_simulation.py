...
random.seed(0)
np.random.seed(0)
analysis_tuple = namedtuple(
    "analysis_tuple",
    "obv_count rev_count edge_count o_deg_min o_deg_mean o_deg_max r_deg_min r_deg_mean r_deg_max o_av_cluster r_av_cluster connect min_comp max_comp spl diam density planar",
)
...
parser.add_argument(
    ...
    default=450,
    type=int,
    help="die lifetime in minutes",
    metavar="Average die lifetime",
)
parser.add_argument(
    "-a",
    default=1.5,
    type=float,
    help="how much longer obverses last than reverses",
...
parser.add_argument(
    ...
    default=600,
    type=int,
    help="length of work period in minutes",
    metavar="Work period",
)
parser.add_argument(
    "-D",
    default=20,
    type=int,
    help="maximum number of obverse dies to be generated",
    metavar="D-max",
)
parser.add_argument(
    "-o_loss",
    default=0,
    type=float,
    help="percentage of obverse dies to delete from full chart",
    metavar="Obv loss",
)
parser.add_argument(
    "-r_loss",
    default=0,
    type=float,
    help="percentage of reverse dies to delete from full chart",
    metavar="Rev loss",
)
parser.add_argument(
    "-e_loss",
    default=0,
    type=float,
    help="percentage of edges to delete from full chart",
    metavar="Edge loss",
)
...
def disruption(G, o_loss, r_loss, e_loss):
    ...
    n_to_delete = list(
        np.random.choice(
            list(die_lifetimes["Obverses"].keys()),
            size=count_obvs_to_delete,
            replace=False,
            p=o_probs,
        )
    )
    for x in list(
        np.random.choice(
            list(die_lifetimes["Reverses"].keys()),
            size=count_revs_to_delete,
            replace=False,
            p=r_probs,
        )
    ):
    ...
    for e in list(
        np.random.choice(
            len(G.edges()), size=count_edges_to_delete, replace=False, p=e_probs
        )
    ):
...
for j in range(0, i):
    ...
    sheet2["R" + str(j + 2)] = analysis_output.edge_count / (
        analysis_output.obv_count + analysis_output.rev_count
    )
